...
class delete():
	try:
		def __init__(self):
			emp= input("rnter the employee id ")
			self.deleteuser(emp)
		def deleteuser(self,emp):
			sql_delete_query = """Delete from register where student_id = %s"""
			cursor.execute(sql_delete_query, (emp,))
			conn.commit()
